...
> **客户端侧的修复必须在 DNS 段，不能只放路由规则。** TUN 模式（本项目推荐的用法）下，
> 应用是**先查 DNS、再发起连接**的 —— 查询在 `ipv4_only` 下已经返回空，路由规则根本轮不到。
> 所以客户端模板在 `dns.rules` 里给该后缀单独开了 `prefer_ipv4`（经节点解析）；
> 路由层那条把它钉到节点的规则一并保留，成本为零且语义明确。改这段时两处都别删。
